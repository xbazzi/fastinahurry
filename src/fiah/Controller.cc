...
bool
Controller::init_client () noexcept
{
...
bool
Controller::init_server () noexcept
{
...
bool
Controller::start_server () noexcept
{
    try
    {
    ...
    {
        LOG_ERROR ("Controller failed to start server (fiah::AlgoException): ",
                   e.what ());
        return 1;
    }
    catch (std::exception &e)
    {
        LOG_ERROR ("Controller failed to start server (std::exception): ",
                   e.what ());
        return 1;
    }
}
...
bool
Controller::start_client () noexcept
{
    try
    {
    ...
    {
        LOG_ERROR ("Controller failed to start client (fiah::AlgoException): ",
                   e.what ());
        return 1;
    }
    catch (std::exception &e)
    {
        LOG_ERROR ("Controller failed to start client (std::exception): ",
                   e.what ());
        return 1;
    }
}
