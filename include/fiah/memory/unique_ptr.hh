...
/// @brief Custom unique Pointer_T implementation as an alternative to std::unique_ptr
...
/// It is move-only and cannot be copied. When the unique_ptr is destroyed or
/// reset, it calls the deleter on the managed object.
///
/// Example usage:
/// @code
/// ```
///   auto ptr = fiah::memory::unique_ptr<int>(new int(42));
///   *ptr = 10;
///   int value = *ptr;
///
///   auto custom = fiah::memory::unique_ptr<int, CustomDeleter>(new int(5), CustomDeleter{});
/// ```
/// @endcode
///
...
/// @brief Create a unique_ptr with the given arguments
