...
class TcpServer : public Tcp
{
  private:
    ...
    static inline fiah::Logger<TcpServer> &m_logger{fiah::Logger<TcpServer>::get_instance("TcpServer")};
...
inline auto TcpServer::start() -> std::expected<void, fiah::TcpError>
{
    ...
    if (::bind(m_sock, reinterpret_cast<sockaddr *>(&addr), sizeof(addr)) < 0)
    {
        ...
        return std::unexpected(fiah::TcpError::BIND_FAIL);
    }

    if (::listen(m_sock, MAX_LISTEN_NUM) < 0)
    {
        LOG_ERROR("Couldn't listen on socket.");
        return std::unexpected(fiah::TcpError::LISTEN_FAIL);
    }
